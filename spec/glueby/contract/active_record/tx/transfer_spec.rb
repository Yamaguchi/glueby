...
RSpec.describe 'Glueby::Contract::AR::Tx::Transfer' do
  def setup_database
    ::ActiveRecord::Base.establish_connection(config)
    connection = ::ActiveRecord::Base.connection
    connection.create_table :transfers do |t|
    ...
    connection.add_index :transfers, [:txid], unique: true
  ...
  after do
    connection = ::ActiveRecord::Base.connection
    connection.drop_table :transfers, if_exists: true
  ...
  describe '#valid?' do
    ...
    let(:tx) { Glueby::Contract::AR::Tx::Transfer.new(sender_wallet_id: sender_wallet_id, receiver_wallet_id: receiver_wallet_id, amount: amount, txid: txid, status: status, token: token) }
